...
router.get("/stocks/quotes", async (req, res) => {
  ...
  try {
    const results = parsed.map((p) => {
      ...
      if (isKorean) {
        const q = krMap[p.ticker] as KrQuote | undefined;
...
router.get("/stocks/screen", async (req, res) => {
  ...
  let krFdrMap: Record<string, KrQuote> = {};
  if (krCandidates.length > 0) {
    const yMap = await yahooKrMultiQuote(krCandidates.map(c => ({ ticker: c.ticker, market: c.market })));
...
router.get("/stocks/analyze", async (req, res) => {
  ...
  try {
    ...
    // 현재가/52w: Yahoo Finance
    const currentPrice: number = isKorean
      ? ((krQ as any)?.close ?? pr?.regularMarketPrice ?? 0)
