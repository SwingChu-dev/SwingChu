...
export const STOCKS: StockInfo[] = [
  ...
  },
  {
    ...
    region: "미국장",
    grade: "소형주",
    themes: ["에너지", "그린란드", "희토류", "자원개발", "트럼프테마"],
    currentPrice: Math.round(5 * USD_KRW_RATE),
    currency: "KRW",
    description: "그린란드 에너지 자원 개발 전문 기업. 트럼프 행정부의 그린란드 전략 자산화 정책 수혜 기대. 희토류·우라늄·석유 등 그린란드 풍부한 지하자원 탐사·개발.",
    splitEntries: [
      { ratio: 30, dropPercent: 10, targetPrice: Math.round(5 * USD_KRW_RATE * 0.90) },
      { ratio: 30, dropPercent: 20, targetPrice: Math.round(5 * USD_KRW_RATE * 0.80) },
      { ratio: 40, dropPercent: 30, targetPrice: Math.round(5 * USD_KRW_RATE * 0.70) },
    ],
    profitTargets: [
      { percent: 15, price: Math.round(5 * USD_KRW_RATE * 1.15) },
      { percent: 30, price: Math.round(5 * USD_KRW_RATE * 1.30) },
      { percent: 60, price: Math.round(5 * USD_KRW_RATE * 1.60) },
    ],
    boxRange: {
      support:         Math.round(3.5 * USD_KRW_RATE),
      resistance:      Math.round(8 * USD_KRW_RATE),
      currentPosition: "중간권",
    },
    forecasts: [
      { period: "1일 후",    price: Math.round(5 * USD_KRW_RATE * 1.01),  changePercent: 1.0   },
      { period: "1주 후",    price: Math.round(5 * USD_KRW_RATE * 1.05),  changePercent: 5.0   },
      { period: "1개월 후",  price: Math.round(5 * USD_KRW_RATE * 1.20),  changePercent: 20.0  },
      { period: "3개월 후",  price: Math.round(5 * USD_KRW_RATE * 1.50),  changePercent: 50.0  },
      { period: "6개월 후",  price: Math.round(5 * USD_KRW_RATE * 2.00),  changePercent: 100.0 },
      { period: "12개월 후", price: Math.round(5 * USD_KRW_RATE * 3.00),  changePercent: 200.0 },
      { period: "1800일",    price: Math.round(5 * USD_KRW_RATE * 10.0),  changePercent: 900.0 },
    ],
    financials: {
      per: null,
      pbr: 2.5,
      ...
      revenueGrowth: 0,
      evaluation: "성장주",
      summary: "초기 탐사 단계 소형주로 적자 구조. 그린란드 자원 개발 정책 수혜 모멘텀이 핵심 투자 논거. 트럼프 행정부 정책 강도에 따라 변동성 극심.",
    },
    dayFeatures: [
      { day: "월요일", feature: "그린란드 관련 지정학 뉴스 반응", caution: "트럼프 발언·외교 뉴스에 급등락" },
      { day: "화요일", feature: "자원개발 섹터 뉴스 동조", caution: "희토류·우라늄 가격 연동" },
      { day: "수요일", feature: "미국 에너지 정책 발표일", caution: "IEA·EIA 보고서 영향" },
      { day: "목요일", feature: "소형주 기관 매매 패턴", caution: "유동성 낮아 급격한 변동 주의" },
      { day: "금요일", feature: "주간 포지션 정리 소형주 변동성", caution: "저유동성 + 주말 뉴스 리스크" },
    ],
    risk: {
      geopolitical: "그린란드 덴마크 주권 분쟁 심화 시 탐사 허가 불확실성. 트럼프 정책 후퇴 시 급락 위험.",
      technicalBounce: "소형주로 지지선 파악 어려움. 진입 전 반드시 소량 테스트 매수.",
      strategy: "포트 3% 이내 투기적 소액 배분. 단기 모멘텀 매매 전략. 장기 보유 시 그린란드 자원 정책 확정 여부 모니터링 필수.",
    },
    witchDayStrategy: "소형 에너지 소재주로 옵션 만기 영향 미미. 만기일 코스피 급등락과 무관하게 독자적 뉴스 중심 움직임.",
    entryRecommendation: "트럼프 그린란드 관련 발언·행정명령 이후 모멘텀 진입. 소액 분산 원칙. 손절선 -25% 엄수.",
  },
];
